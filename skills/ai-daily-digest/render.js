...
function renderWkhtml(card, outFile, tmpDir){
  ...
  const args = ["--enable-local-file-access","--encoding","utf-8","--quality","100",
...
async function renderPlaywright(cards, outDir){
  ...
  for (const card of cards){
    ...
    await el.screenshot({path: path.join(outDir, card.name + ".png")});
...
(async function main(){
  ...
  if (engine==="playwright"){ await renderPlaywright(cards, outDir); }
    ...
    for (const card of cards){
      renderWkhtml(card, path.join(outDir, card.name + ".png"), tmpDir);
      console.log("  ✓", card.name + ".png");
    }
  }
  fs.writeFileSync(path.join(outDir,"manifest.json"),
    JSON.stringify({date:DATA.date, cnDate:DATA.cnDate, files:cards.map(c=>c.name+".png")}, null, 2));
})();
