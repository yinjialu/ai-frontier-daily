...
(function (root) {
  ...
  function buildDeck(DATA){
    var out=[], U=DATA.updates||[];

    // 小红书封面
    out.push({name:"小红书_00_封面", cls:"xhs xhs-cover", w:1080, h:1440, inner:`
      <div class="blob" style="width:440px;height:440px;background:var(--clay-soft);top:-120px;right:-100px"></div>
      <div class="blob" style="width:220px;height:220px;background:var(--clay);bottom:520px;left:-80px;opacity:.45"></div>
      <div class="inner">
        <div class="topbar"><span class="edition">${esc(DATA.edition)}</span><span class="date">${esc(DATA.date)}</span></div>
        <div class="kicker">Anthropic Daily</div>
        <h1>今天 Anthropic<br>又<em>更新</em>了什么</h1>
        <div class="teasers">
    ...
    // 小红书结尾卡
    out.push({name:"小红书_99_结尾", cls:"xhs xhs-outro", w:1080, h:1440, inner:`
      <div class="blob" style="width:400px;height:400px;background:var(--clay);bottom:-120px;right:-100px;opacity:.4"></div>
      <div class="inner">
    ...
    // 公众号封面
    out.push({name:"公众号_封面_900x383", cls:"gzh-cover", w:900, h:383, inner:`
      <div class="blob" style="width:340px;height:340px;background:var(--clay);top:-130px;right:140px;opacity:.45"></div>
      <div class="inner">
        <div class="row"><span>${esc(DATA.edition)} · ANTHROPIC DAILY</span><span>${esc(DATA.date)}</span></div>
        <h1>Anthropic 今日<em> 速递</em></h1>
        <div class="sub">${esc(DATA.cnDate)} · 共 ${U.length} 条更新 · ${esc(DATA.brand)}</div>
      </div>`});

    // 公众号正文长图（高度自适应）
    out.push({name:"公众号_正文长图", cls:"gzh-body", w:900, h:null, inner:`
      <div class="inner">
  ...
  root.Cards = { buildDeck: buildDeck };
