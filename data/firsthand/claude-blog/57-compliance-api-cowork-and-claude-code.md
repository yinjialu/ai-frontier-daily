...
source: claude-blog
resource: https://claude.com/blog/compliance-api-cowork-and-claude-code
published: 2026-08-11
tags: [Compliance API, Claude Code, Claude Cowork, 企业合规]
detected: 2026-08-17T07:39:55+08:00
---

Anthropic 将 Compliance API 覆盖扩展到 Claude Cowork 和 Claude Code，企业客户可通过同一接口获取会话内容与元数据，用于审计和 eDiscovery，新增端点不改变现有数据，支持与 OpenTelemetry 并行。
...
Session content:
prompts and responses, tool calls content (web and MCP), and skills and artifacts content captured as transcript text.
Session metadata:
verified user ID and email address, organization ID, session and per-message IDs, and timestamps.
...
Getting started
Coverage for Cowork and Claude Code is available today and included with the Compliance API using your existing Compliance Access Key – there’s no separate integration to build. If it's already enabled for your organization, query the new session endpoints directly. If not, review the Compliance API
documentation
to enable it.
No items found.
Prev
