...
## 四、wechat2rss / 公众号：不做（结论存档，别再翻案）

### wechat2rss 原理剖析（核心闭源，但已查清）
- GitHub 仓 `ttttmr/Wechat2RSS` **不含抓取引擎**——只是 vitepress 文档站 + 300+ 公众号列表；
  核心是**付费闭源**（"私有部署，功能与公开服务一致，点击购买"），thanks RSSHub。
- 难点：公众号文章列表在 `mp.weixin.qq.com`，**无公开 API**。业界仅两条路：
  1. **搜狗微信** weixin.sogou.com：搜索引擎索引，无需登录，但强反爬（几次即验证码）、只给最近~10篇、易失效。
  2. **微信历史消息接口** mp/profile_ext：需真实微信会话 `biz`+`pass_ticket`+`appmsg_token`+cookie，
     靠 mitmproxy 抓真机微信请求取得，token 几小时~几天过期要刷新（"养号"），可靠但维护极重。
- wechat2rss 的价值 = 帮你维护方案2的"养号 + token 刷新"基建 → 这就是它能"长期稳定"且收费的原因。

### 自建 vs 简化版评估
- "自建 wechat2rss" = 买**付费私有部署**（非免费开源能跑）。
- "做简化版"：搜狗路线**不稳定**（违背"稳定"核心目标）；mitmproxy 养号路线**维护极重**（个人系统不划算）。

### 决定：不做（维持现状）
公众号同时踩两条原则雷区——**不稳定**（无 API/反爬/token 过期）+ **二手**（晚点LatePost 是财经媒体非官方一手）。
- 内参守"官方一手 + 稳定"纪律，**公众号/媒体号不进 firsthand**。
- 晚点这类**媒体**真要看 → 归**早报的 WebSearch 层**（早报本就混源策展），不为它建不稳的抓取基建。
- 仅极少数刚需号才考虑买 wechat2rss 付费服务（花钱省心，但仍第三方依赖 + 二手）。

（实测补充：xlab.app 免费托管未收录晚点；latepost.com 官网纯 JS 渲染、无 RSS、无可见 API。）
...
- ~~**wechat2rss / 公众号**~~：**已结论：不做**（不稳定 + 二手，见第四节）。媒体号交给早报 WebSearch。
- **阶段二 browser 适配器**：智谱 GLM、Kimi 等**官方一手**纯 JS 渲染源（参考 `x-fetch.sh` 的 agent-browser 范式）。
  注意：仅限官方源；晚点等媒体号不在此列。
